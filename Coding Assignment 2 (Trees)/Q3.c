/*
Write a C program that takes inorder and postorder traversal as input,
...
void isPossibleTree(int* postorder, int* inorder, int postSize, int inSize)
{
    if(postSize != inSize)
    {
        printf("Invalid input");
...
node* buildTree(int* postorder, int* inorder, int postSize, int inSize)
{
    isPossibleTree(postorder, inorder, postSize, inSize);
    if (postSize == 0)
        return NULL;

    node* root = createNode(postorder[postSize-1]);
    int i = 0;

    while (inorder[i] != postorder[postSize-1])
        i++;

    root->left = buildTree(postorder, inorder, i, i);
    root->right = buildTree(postorder + i, inorder + i + 1, postSize - i - 1, postSize - i - 1);
...
int main() 
{
    ...
    node* root = buildTree(postorder, inorder, postSize, inSize);
