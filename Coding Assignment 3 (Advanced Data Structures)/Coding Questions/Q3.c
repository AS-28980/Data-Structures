...
struct _234Node 
{
    ...
    int count;
    int data[3];
    struct _234Node* children[4];
};
typedef struct _234Node node;

struct _234tree 
{
    node* root;
};
typedef struct _234tree tree;
...
node* movekeyFromLeftSibling(node* x, int i) 
{
    ...
    x->data[i - 1] = leftSibling->data[leftSibling->count - 1];
...
node* movekeyFromRightSibling(node* x, int i) {
    ...
    child->data[child->count] = x->data[i];

    if (!child->isLeaf) 
        child->children[child->count + 1] = rightSibling->children[0];

    x->data[i] = rightSibling->data[0];

    for (int j = 0; j < rightSibling->count - 1; j++) 
        rightSibling->data[j] = rightSibling->data[j + 1];
...
{
    int idx = 0;
    while (idx < x->count && k > x->data[idx]) 
        idx++;
...
node* delete(node* x, int k) 
{
    ...
    if (pos < x->count && x->data[pos] == k) 
    {
        ...
        if (!x->isLeaf) 
        {
            if (x->children[pos]->count >= 2) 
            {
                int pred = Pred(x, pos);
                x->data[pos] = pred;
                x->children[pos] = delete(x->children[pos], pred);
            } 
            else if (x->children[pos + 1]->count >= 2) 
            {
                int succ = Succ(x, pos);
                x->data[pos] = succ;
    ...
    else 
    {
        ...
        {
            if (i > 0 && x->children[i - 1]->count >= 2)
                x = movekeyFromLeftSibling(x, i);
            else if (i < x->count && x->children[i + 1]->count >= 2)
...
int main() 
{
    ...
    for(int i=0; i<n; i++)
        _tree->root = insert(_tree->root, elements[i]);
    ...
    while(1) 
    {
        ...
        switch (queryChoice) 
        {
            ...
            case 2:
                ...
                {
                    _tree->root = insert(_tree->root, query);
                    printf("%d inserted\n", query);
                }
                break;
            ...
            case 3:
                if(search(_tree->root, query))
                {
                    _tree->root = delete(_tree->root, query);
                    printf("%d deleted\n", query);
                }
                else
            ...
            case 4:
                if (_tree->root != NULL) 
                    print234tree(_tree->root);
                else
                    printf("234 tree is empty\n");
                break;
